...
<style>
/* ============================================================
   Monarch India Energy Transition Fund - Design System
   Palette mirrors hw1_index: #3498DB primary blue, #2C3E50 dark,
   white/light-grey backgrounds, clean Segoe UI typography.
   ============================================================ */

/* ── Page layout ─────────────────────────────────────────────────────────── */
.quarto-container { max-width:1200px !important; margin-left:auto; margin-right:auto; }
.page-columns .content { padding-left:1rem !important; padding-right:1rem !important; }

/* ── Base Typography ─────────────────────────────────────────────────── */
body { font-family:'Segoe UI',system-ui,sans-serif; font-size:15px; color:#2c3e50; line-height:1.78; }
h1   { color:#2c3e50; font-weight:700; margin-top:2rem; }
h2   { color:#34495e; font-weight:600; margin-top:1.6rem; }
h3   { color:#2c3e50; font-weight:600; }
h4   { font-weight:600; }
p    { line-height:1.78; }

/* ── Code fold ─────────────────────────────────────────────────────── */
details.code-fold summary { cursor:pointer; list-style:disclosure-closed; color:#6c757d; font-size:0.85em; padding:4px 0; }
details.code-fold[open] summary { list-style:disclosure-open; }
details.code-fold summary::marker { color:#6c757d; }

/* ── Hero banner - light grey gradient + blue left border (like hw1 exec-summary-hero) */
.exec-hero {
  background: linear-gradient(135deg, #f8f9fa 0%, #ebf5fb 60%, #e8f4f8 100%);
  border-left: 5px solid #3498db;
  color: #2c3e50;
  padding: 28px 36px;
  border-radius: 0 12px 12px 0;
  margin: 20px 0 30px;
  box-shadow: 0 3px 16px rgba(52,152,219,0.12);
}
.exec-hero h2  { color:#2c3e50; border:none; font-size:1.36em; margin-top:0; margin-bottom:14px; font-weight:700; padding-left:0; }
.exec-hero h2::before { content: none; }
.exec-hero p   { margin:7px 0; line-height:1.75; font-size:0.97em; color:#34495e; }
.exec-hero .tag { display:inline-block; padding:3px 12px; border-radius:4px; font-weight:600; font-size:0.79em; letter-spacing:0.4px; margin:3px 4px 3px 0; }
.exec-hero .tag-blue  { background:#3498db; color:white; }
.exec-hero .tag-green { background:#27ae60; color:white; }
.exec-hero .tag-amber { background:#e67e22; color:white; }

/* ── KPI metric pills inside hero ──────────────────────────────────────── */
.metric-row { display:grid; grid-template-columns:repeat(auto-fit,minmax(110px,1fr)); gap:10px; margin:18px 0 4px; }
.metric-pill { background:white; border:1px solid #d6eaf8; border-radius:8px; padding:10px 12px; text-align:center; box-shadow:0 1px 4px rgba(52,152,219,0.08); }
.metric-pill .mp-val { font-size:1.48em; font-weight:700; color:#3498db; line-height:1.1; }
.metric-pill .mp-lbl { font-size:0.67em; color:#7f8c8d; margin-top:5px; text-transform:uppercase; letter-spacing:0.6px; }

/* ── Three-test tiles (hw1 card style) ───────────────────────────────── */
.test-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin:20px 0; }
.tile { text-align:center; padding:20px 16px; border-radius:0 8px 8px 0; box-shadow:0 2px 8px rgba(0,0,0,0.06); transition:transform 0.18s; }
.tile:hover { transform:translateY(-2px); }
.tile.pass { background:#f0f9f4; border-left:5px solid #27ae60; }
.tile.cond { background:#fefcf3; border-left:5px solid #f39c12; }
.tile.fail { background:#fdf2f2; border-left:5px solid #e74c3c; }
.tile .icon { font-size:1.9em; margin-bottom:8px; }
.tile h4    { margin:8px 0 6px; font-size:0.93em; color:#2c3e50; font-weight:700; }
.tile p     { margin:0; font-size:0.82em; color:#636e72; line-height:1.55; }

/* ── Cards (mirrors hw1 .info-card / .answer-card / .warning-card) ───────── */
.card { padding:1rem 1.5rem; border-radius:0 6px 6px 0; margin:1.2rem 0; line-height:1.75; }
.card h4 { margin-top:0; margin-bottom:0.5rem; font-weight:600; font-size:1.05rem; }
/* blue card = hw1 .info-card */
.card-blue   { border-left:5px solid #3498db; background-color:#f0f7fc; }
.card-blue h4 { color:#3498db; }
/* green card = hw1 .answer-card */
.card-green  { border-left:5px solid #27ae60; background-color:#f0f9f4; }
.card-green h4 { color:#27ae60; }
/* red card = hw1 .critical-card */
.card-red    { border-left:5px solid #e74c3c; background-color:#fdf2f2; }
.card-red h4 { color:#e74c3c; }
/* amber card = hw1 .warning-card */
.card-amber  { border-left:5px solid #f39c12; background-color:#fefcf3; }
.card-amber h4 { color:#e67e22; }
.card-purple { border-left:5px solid #9b59b6; background-color:#f5eef8; }
.card-purple h4 { color:#9b59b6; }
/* navy card = hw1-ish: light blue bg */
.card-navy   { border-left:5px solid #2980b9; background-color:#eaf4fc; box-shadow:0 1px 6px rgba(52,152,219,0.08); }
.card-navy h4 { color:#2c3e50; }

/* ── Collapsible details ──────────────────────────────────────────────── */
details.section-fold { margin:14px 0; border-radius:6px; overflow:hidden; border:1px solid #d6eaf8; }
details.section-fold > summary { cursor:pointer; list-style:none; padding:11px 18px; background:#f0f7fc; border-left:4px solid #3498db; font-weight:600; font-size:0.93em; color:#2c3e50; user-select:none; }
details.section-fold > summary::-webkit-details-marker { display:none; }
details.section-fold > summary::before { content:"\25B6  "; font-size:0.75em; color:#3498db; }
details.section-fold[open] > summary { background:#d6eaf8; }
details.section-fold[open] > summary::before { content:"\25BC  "; }
details.section-fold > .body { padding:16px 22px; background:white; font-size:0.93em; line-height:1.75; border-top:1px solid #d6eaf8; }

/* ── Callout overrides (same as hw1) ───────────────────────────────── */
.callout-tip      { background-color:transparent !important; border-left-color:#3498db !important; }
.callout-tip .callout-title { color:#2780e3 !important; }
.callout-note     { background-color:transparent !important; }
.callout-note .callout-title { color:#2780e3 !important; }
.callout-warning  { background-color:transparent !important; }
.callout-warning .callout-title { color:#e67e22 !important; }
.callout-important .callout-title { color:#e74c3c !important; }
.callout-note.callout-style-default     > .callout-header { background-color:rgb(233,242,252) !important; }
.callout-tip.callout-style-default      > .callout-header { background-color:rgb(233,242,252) !important; }
.callout-warning.callout-style-default  > .callout-header { background-color:rgb(255,243,224) !important; }
.callout-important.callout-style-default > .callout-header { background-color:rgb(253,229,229) !important; }

/* ── Section divider - grey fade (exactly hw1's .section-divider) ─────── */
.divider { border:none; height:2px; background:linear-gradient(90deg,transparent,#bdc3c7,transparent); margin:2.5rem 0; }

/* ── Tables (hw1 .styled-table) ────────────────────────────────────────── */
.gt { width:100%; border-collapse:collapse; font-size:0.92em; margin:1rem 0; }
.gt thead tr { background:#2c3e50; color:white; }
.gt th { padding:10px 14px; text-align:left; font-weight:600; }
.gt td { padding:8px 12px; border-bottom:1px solid #ddd; }
.gt tbody tr:nth-child(even) { background:#f8f9fa; }
.gt tbody tr:hover { background:#e8f4f8; }

/* ── Badges ─────────────────────────────────────────────────────────── */
.screen-badge { display:inline-block; padding:3px 10px; border-radius:4px; font-size:0.79em; font-weight:600; margin:2px; }
...
.star-low  { color:#95a5a6; font-weight:600; }

/* ── Watermark ───────────────────────────────────────────────────── */
@media screen {
  body::after {
    content:"TEAM: Mountains & Accretions\AIIM Indore ";
    position:fixed; bottom:5px; right:10px;
    font-size:11px; color:rgba(100,100,100,0.35);
    font-family:'Segoe UI',Arial,sans-serif; pointer-events:none;
    white-space:pre; text-align:right; z-index:1000;
  }
}

/* ── Top navigation bar – clean white/cosmo style ───────────────── */
.navbar {
...
.navbar .navbar-brand { color: #2c3e50 !important; font-weight: 700; font-size: 1.05rem; letter-spacing: 0.2px; }
.navbar .navbar-brand:hover { color: #3498db !important; }
.navbar .nav-link { color: #495057 !important; font-size: 0.9rem; }
.navbar .nav-link:hover { color: #3498db !important; }
/* Quarto sidebar TOC header */
#quarto-sidebar .sidebar-title { font-weight: 700; color: #2c3e50; }
#quarto-sidebar .nav-link { font-size: 0.85rem; color: #495057; }
#quarto-sidebar .nav-link:hover { color: #3498db; }
#quarto-sidebar .active > .nav-link { color: #3498db !important; font-weight: 600; border-left: 3px solid #3498db; background: #f0f7fc; }
/* Document title area - styled but visible */
.quarto-title-meta { font-size: 0.88em; color: #6c757d; }
.quarto-title h1.title { color: #2c3e50; font-weight: 700; margin-bottom: 0.3rem; }
.quarto-title .subtitle { color: #3498db; font-weight: 500; }

/* ── Print ───────────────────────────────────────────────────────── */
@media print {
  .navbar { background: #2c3e50 !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Fire on initial page load (matches hw1_index.qmd pattern)
  ...
  // Re-render when details/summary sections open
  document.querySelectorAll('details').forEach(function(details) {
    details.addEventListener('toggle', function() {
      if (details.open) { setTimeout(resizePlots, 150); }
    });
  });
});
</script>
...
# ── Colour palette - mirrors hw1_index.qmd (pastel blue, clean) ────────────
C = dict(
    navy   = '#2c3e50',   # dark slate - headings, chart text
    blue   = '#3498db',   # primary blue (hw1 primary_blue)
    green  = '#27ae60',   # positive
    amber  = '#f39c12',   # warning
    red    = '#e74c3c',   # negative
    purple = '#9b59b6',   # deep dive
    teal   = '#1abc9c',   # secondary accent
    orange = '#e67e22',   # warning cards
    lt_blue= '#85c1e9',   # light blue fill
    lt_grn = '#a9dfbf',   # light green fill
    bg     = '#f8f9fa',   # light background
    forest  = '#2c3e50',  # alias -- used in BL() titles
    emerald = '#3498db',  # alias -- used in SA() grid accent
    gold    = '#f39c12',  # alias -- used in highlight annotations
)
...
# ── Chart layout helper - matches hw1 clean white chart style ──────────
def BL(title_text, height=440, legend_y=1.09, extra_t=0, **kw):
    return dict(
        title=dict(text=title_text, x=0.0, xanchor='left',
                   font=dict(size=15, color='#2C3E50', family='Segoe UI')),
        height=height,
        margin=dict(t=130 + extra_t, b=62, l=65, r=45),
        paper_bgcolor='white',
        plot_bgcolor='#F8F9FA',
...
def SA(fig, xgrid=True, ygrid=True):
    """Axis styler - clean grey grid matching hw1 Plotly charts."""
    fig.update_xaxes(
        showgrid=xgrid, gridcolor='#ecf0f1', gridwidth=1,
        zeroline=True, zerolinecolor='#dee2e6', zerolinewidth=1,
        linecolor='#dee2e6', linewidth=1, showline=True,
    )
    fig.update_yaxes(
        showgrid=ygrid, gridcolor='#ecf0f1', gridwidth=1,
        zeroline=True, zerolinecolor='#dee2e6', zerolinewidth=1,
        linecolor='#dee2e6', linewidth=1, showline=True,
    )
    return fig
...
<p>
<span class="tag tag-blue">📈 Sectoral Thematic &middot; Energy Transition</span>
<span class="tag tag-blue">NSE India Universe</span>
<span class="tag tag-green">Monarch AMC &middot; Rs {AUM_CR} Cr AUM (Hypothetical)</span>
<span class="tag tag-amber">Benchmark: Nifty 50 &nbsp;|&nbsp; {n_stocks} Holdings</span>
</p>
<p style="margin-top:14px; color:#34495e;">
India’s energy transition is the largest capital reallocation event of this decade. The government
has committed to <strong>500 GW of non-fossil capacity by 2030</strong>, implying
Rs 25&ndash;30 lakh crore of infrastructure investment over six years. This fund captures that
structural shift across the full value chain &mdash; renewable developers, grid utilities,
industrial capital goods &amp; EPC, energy-efficient systems, and water infrastructure.
</p>
<div class="metric-row">
  <div class="metric-pill"><div class="mp-val">{port_ann_pct}%</div><div class="mp-lbl">Ann. Return (p.a.)</div></div>
  <div class="metric-pill"><div class="mp-val">{port_sharpe_v}</div><div class="mp-lbl">Sharpe Ratio</div></div>
  <div class="metric-pill"><div class="mp-val">{port_sortino_v}</div><div class="mp-lbl">Sortino Ratio</div></div>
  <div class="metric-pill"><div class="mp-val">{wa_roce}%</div><div class="mp-lbl">Wtd. Avg ROCE</div></div>
  <div class="metric-pill"><div class="mp-val">{port_beta_v}</div><div class="mp-lbl">Portfolio Beta</div></div>
  <div class="metric-pill"><div class="mp-val">{port_mdd_pct}%</div><div class="mp-lbl">Max Drawdown</div></div>
  <div class="metric-pill"><div class="mp-val">{total_ret_pct}%</div><div class="mp-lbl">Total Return (5Y)</div></div>
  <div class="metric-pill"><div class="mp-val">{port_ir_v}</div><div class="mp-lbl">Info. Ratio</div></div>
</div>
<p style="font-size:0.83em; color:#7f8c8d; margin-top:14px;">
<strong>Team Mountains &amp; Accretions</strong> &nbsp;&middot;&nbsp; IIM Indore &nbsp;&middot;&nbsp;
Developed for: IIM-A Launchpad Alpha &nbsp;&middot;&nbsp; Data through {data_end}
</p>
</div>
"""
...
# ── Build 3-level sunburst (Fund → Sector → Stock) ────────────────────────
...
for sec in sorted(set(sectors.values())):
    ...
    sb_values.append(0.0); sb_colors.append(SC.get(sec, C['navy']))
...
fig = go.Figure(go.Sunburst(
    ...
    branchvalues='remainder', rotation=90,
    hovertemplate='<b>%{label}</b><br>Weight: %{value:.2f}%<extra></extra>',
))
fig.update_layout(
    title=dict(
        text='Capital Allocation Hierarchy - Sunburst View<br>'
...
## 4c. Efficient Frontier & Random Portfolio Cloud

```{python}
#| label: fig-efficient-frontier
#| fig-cap: "Efficient frontier - 5,000 random portfolios coloured by Sharpe ratio; actual fund portfolio marked"
#| echo: false

if data_ok and len(valid_tickers) >= 4:
    import warnings
    warnings.filterwarnings('ignore')
    from scipy.optimize import minimize

    ret_mat   = daily_ret[valid_tickers].dropna()
    mu_vec    = ret_mat.mean() * TRADING_DAYS           # annualised mean
    cov_mat   = ret_mat.cov()  * TRADING_DAYS           # annualised cov
    n_assets  = len(valid_tickers)

    # ── 5,000 random portfolios ────────────────────────────────────────
    N_RAND = 5000
    np.random.seed(7)
    rand_w   = np.random.dirichlet(np.ones(n_assets), N_RAND)
    rand_ret = rand_w @ mu_vec.values
    rand_var = np.einsum('ij,jk,ik->i', rand_w, cov_mat.values, rand_w)
    rand_vol = np.sqrt(rand_var)
    rand_sh  = (rand_ret - RISK_FREE) / rand_vol

    # ── Actual portfolio ───────────────────────────────────────────────
    w_arr = np.array([WEIGHTS.get(t,0) for t in valid_tickers])
    act_ret = float(w_arr @ mu_vec.values)
    act_vol = float(np.sqrt(w_arr @ cov_mat.values @ w_arr))
    act_sh  = (act_ret - RISK_FREE) / act_vol

    # ── Min-variance frontier (via parametric sweep) ───────────────────
    target_rets = np.linspace(mu_vec.min(), mu_vec.max(), 80)
    ef_vols = []
    for tr in target_rets:
        cons = [{'type':'eq','fun': lambda w, tr=tr: w @ mu_vec.values - tr},
                {'type':'eq','fun': lambda w: w.sum() - 1}]
        bnds = [(0,1)] * n_assets
        res  = minimize(lambda w: w @ cov_mat.values @ w,
    ...
    ef_ret  = target_rets * 100
    ef_vol  = ef_vols    * 100
    mask_ef = ~np.isnan(ef_vol)

    fig = go.Figure()

    # Cloud of random portfolios (coloured by Sharpe)
    fig.add_trace(go.Scatter(
        x=rand_vol*100, y=rand_ret*100,
        mode='markers',
        marker=dict(color=rand_sh, colorscale='RdYlGn', cmin=-0.2, cmax=2.0,
                    size=4, opacity=0.55,
                    colorbar=dict(title=dict(text='Sharpe Ratio'),
                                  thickness=14, len=0.8, x=1.02)),
        name='Random Portfolios',
        hovertemplate='Vol: %{x:.1f}%<br>Ret: %{y:.1f}%<extra>Random</extra>',
        showlegend=True,
    ))

    # Efficient frontier line
    fig.add_trace(go.Scatter(
        x=ef_vol[mask_ef], y=ef_ret[mask_ef],
        mode='lines', name='Efficient Frontier',
        line=dict(color=C['navy'], width=3),
        hovertemplate='EF Vol: %{x:.1f}%<br>EF Ret: %{y:.1f}%<extra></extra>',
    ))

    # Capital Market Line
    max_vol_cml = rand_vol.max()*100 + 5
    cml_x_ef = [0, max_vol_cml]
    cml_y_ef = [RISK_FREE*100, RISK_FREE*100 + act_sh * max_vol_cml]
    fig.add_trace(go.Scatter(
        x=cml_x_ef, y=cml_y_ef, mode='lines',
        line=dict(color=C['teal'], width=1.5, dash='dash'),
        name=f'Capital Market Line (Sharpe={act_sh:.2f})',
    ))

    # Actual portfolio star
    fig.add_trace(go.Scatter(
        x=[act_vol*100], y=[act_ret*100],
        mode='markers+text',
        marker=dict(symbol='star', size=26, color=C['navy'],
                    line=dict(color='white', width=2)),
        text=['Monarch Fund'], textposition='top right',
        textfont=dict(size=11, color=C['navy'], family='Segoe UI'),
        name=f'Monarch Fund (Sharpe={act_sh:.2f})',
        hovertemplate=f'<b>Monarch Fund</b><br>Return: {act_ret*100:.1f}%<br>Vol: {act_vol*100:.1f}%<br>Sharpe: {act_sh:.2f}<extra></extra>',
    ))

    # Nifty 50 diamond
    nifty_vol_ann = float(nifty_ret.std() * np.sqrt(TRADING_DAYS))
    nifty_ret_ann = float((1 + nifty_ret.mean())**TRADING_DAYS - 1)
    fig.add_trace(go.Scatter(
        x=[nifty_vol_ann*100], y=[nifty_ret_ann*100],
        mode='markers+text',
        marker=dict(symbol='diamond', size=16, color=C['red'],
                    line=dict(color='white', width=2)),
        text=['Nifty 50'], textposition='top left',
        textfont=dict(size=10, color=C['red']),
        name='Nifty 50',
        hovertemplate=f'<b>Nifty 50</b><br>Return: {nifty_ret_ann*100:.1f}%<br>Vol: {nifty_vol_ann*100:.1f}%<extra></extra>',
    ))

    fig.update_layout(**BL(
        'Mean-Variance Efficient Frontier - Energy Transition Universe<br>'
        f'<sup>5,000 random portfolios coloured by Sharpe | '
        f'Monarch Fund Sharpe = {act_sh:.2f} | Long-only allocations only</sup>',
        height=520, legend_y=1.05,
    ))
    SA(fig)
    fig.update_xaxes(title_text='Annualised Volatility (%)', ticksuffix='%', range=[0, rand_vol.max()*100+3])
    fig.update_yaxes(title_text='Annualised Return (%)', ticksuffix='%')
    fig.show()
```

<div class="card card-blue">
<h4>Efficient Frontier Interpretation</h4>
The scatter confirms the fund's positioning near the efficient frontier - not at the absolute minimum-variance point (which would require near-100% NTPC/PowerGrid), but on the upper-right region where higher quality/momentum names add return at an acceptable volatility cost. The **Capital Market Line** (tangent from the risk-free rate) passes through the Monarch Fund star, indicating optimal Sharpe positioning given the universe constraints.
</div>

## 4d. Portfolio Monthly Return Fingerprint

```{python}
#| label: fig-monthly-heatmap
#| fig-cap: "Monthly return heatmap - portfolio calendar showing best/worst months by year"
#| echo: false

if data_ok:
    monthly_ret = (1 + port_ret).resample('ME').prod() - 1
    ...
    hm_pivot = monthly_df.pivot(index='Year', columns='Month', values='Return') * 100
    hm_pivot = hm_pivot.sort_index(ascending=False)

    z_mat   = hm_pivot.values
    ann_txt = [[f'{v:+.1f}%' if not np.isnan(v) else '' for v in row] for row in z_mat]
    y_labs  = [str(y) for y in hm_pivot.index]
    x_labs  = [month_names[m-1] for m in hm_pivot.columns]

    fig = go.Figure(go.Heatmap(
        z=z_mat, x=x_labs, y=y_labs,
        colorscale='RdYlGn',
        zmid=0, zmin=-15, zmax=20,
        text=ann_txt, texttemplate='%{text}',
        textfont=dict(size=11, color='#2c3e50'),
        colorbar=dict(title=dict(text='Monthly<br>Return (%)'), thickness=14, len=0.85),
        hovertemplate='<b>%{y} %{x}</b><br>Return: %{z:+.2f}%<extra></extra>',
    ))
    fig.update_layout(**BL(
        'Monthly Return Fingerprint - Monarch Fund Calendar<br>'
        '<sup>Green cells = positive months | Red = negative | Sorted newest year first</sup>',
        height=340, extra_t=20,
    ))
...
## 4f. Annual Alpha Attribution Waterfall

```{python}
#| label: fig-alpha-waterfall
#| fig-cap: "Year-by-year alpha (Fund − Nifty 50) stacked into a cumulative alpha bridge chart"
#| echo: false

alpha_yrs  = [y for y in years_list
              if not np.isnan(port_ann_yr.get(y, np.nan))
              and not np.isnan(nifty_ann_yr.get(y, np.nan))]
alpha_vals = [(port_ann_yr[y] - nifty_ann_yr[y])*100 for y in alpha_yrs]
cum_alpha  = list(np.cumsum(alpha_vals))
base_vals  = [0] + cum_alpha[:-1]

bar_colors = [C['green'] if a >= 0 else C['red'] for a in alpha_vals]

fig = make_subplots(
    rows=1, cols=2, column_widths=[0.58, 0.42],
    subplot_titles=['Annual Alpha (Fund \u2212 Nifty 50 %)', 'Cumulative Alpha Bridge'],
    horizontal_spacing=0.10,
)

# Left: annual alpha bars
fig.add_trace(go.Bar(
    x=[str(y) for y in alpha_yrs], y=alpha_vals,
    marker_color=bar_colors,
    text=[f'{v:+.1f}%' for v in alpha_vals], textposition='outside',
    hovertemplate='<b>%{x}</b><br>Alpha: %{y:+.1f}%<extra></extra>',
    name='Annual Alpha',
), row=1, col=1)
fig.add_hline(y=0, line_color='#dee2e6', line_width=1, row=1, col=1)

# Right: waterfall-style cumulative bridge
for i, (yr, alpha, base) in enumerate(zip(alpha_yrs, alpha_vals, base_vals)):
    col = C['green'] if alpha >= 0 else C['red']
    fig.add_trace(go.Bar(
        x=[str(yr)], y=[alpha], base=[base],
        marker_color=col, marker_line_color='white', marker_line_width=1,
        text=[f'{cum_alpha[i]:+.1f}%'], textposition='outside',
        hovertemplate=f'<b>{yr}</b><br>Alpha: {alpha:+.1f}%<br>Cumulative: {cum_alpha[i]:+.1f}%<extra></extra>',
        showlegend=False, name=str(yr),
    ), row=1, col=2)
fig.add_hline(y=0, line_color='#dee2e6', line_width=1, row=1, col=2)

fig.update_layout(**BL(
    f'Alpha Attribution \u2014 Monarch Fund vs Nifty 50<br>'
    f'<sup>Total cumulative alpha: {sum(alpha_vals):+.1f}% | '
    f'Years with positive alpha: {sum(1 for a in alpha_vals if a>0)}/{len(alpha_vals)}</sup>',
    height=430, legend_y=1.05,
))
SA(fig, xgrid=False)
fig.update_yaxes(title_text='Alpha (%)', ticksuffix='%', row=1, col=1)
fig.update_yaxes(title_text='Cumulative Alpha (%)', ticksuffix='%', row=1, col=2)
fig.show()
```
...
if data_ok:
    ...
    fig.add_trace(go.Histogram(
        ...
        marker_color=C['navy'], opacity=0.55, histnorm='probability density',
    ...
    fig.update_layout(**BL(
        f'Return Distribution & Tail Risk - Portfolio Daily Returns<br>'
...
The QQ-plot tail departure confirms the fund's daily return distribution has **higher kurtosis than a Gaussian** - meaning extreme return days (both positive and negative) occur more frequently than the normal curve predicts. This is typical of equity portfolios with thematic concentration. The practical implication: our parametric VaR95 (based on the normal assumption) likely **understates** true tail risk by 15–25%. CVaR95 (which captures the average beyond the VaR threshold) is the more conservative metric to monitor. Sharpe ratio remains valid as a relative measure, but absolute risk estimates should be treated as lower bounds.
</div>
...
# Component VaR_i = w_i × ρ(r_i, r_p) × σ_i × z_95 × √252
...
# Low Volatility: 1/(ann vol %) - lower vol = higher low-vol score
fac_rows = []
for t in valid_tickers:
    f    = FUNDAMENTALS.get(t, (15,30,0,2))
    roce, pe = f[0], f[1]
...
# Scenario 1 - Market crash (Nifty -25%): stock_impact ≈ beta_i × -25%
# Scenario 2 - Rate hike +150bps: duration_proxy × (-0.015); 
#              utilities shorter duration (regulated tariffs), industrials longer
# Scenario 3 - Commodity spike (steel/copper +40%): affects EPC margin (input cost %)
# Scenario 4 - Policy reversal (RE tariff freeze / subsidy cut ~30%):
...
# ── Attribution (return to price target, 3-yr horizon) ────────────────────
attr_rows = []
...
#| fig-cap: "Pairwise return correlation matrix - lower-triangle; sorted by sector"
#| echo: false
...
if data_ok:
    ...
    fig.update_layout(**BL(
        'Return Correlation Matrix - Energy Transition Fund Universe<br>'
        '<sup>Lower triangle | Sorted by sector grouping | Jan 2020–Feb 2026</sup>',
...
Within-sector correlations are predictably high (Solar/Wind stocks cluster at 0.55–0.75) while cross-sector correlations are more moderate. The most important observation: **NTPC and Power Grid** - the fund's two largest anchor positions - show relatively low correlation to pure-play RE names (r ≈ 0.35–0.45), confirming their diversification benefit within the portfolio. The EV/Components (Motherson) and Water/Waste (Wabag) allocations provide genuine cross-sectoral diversification.
</div>

## 6b. Fundamental Quality Map - ROCE vs P/E

```{python}
#| label: fig-quality-map
#| fig-cap: "Fundamental quality map: ROCE (quality) vs P/E (valuation) - ideal holdings are top-left (high ROCE, low P/E); bubble size = portfolio weight"
#| echo: false

fig = go.Figure()

# Quadrant shading
max_pe = max(FUNDAMENTALS.get(t,(0,40,0,0))[1] for t in valid_tickers if FUNDAMENTALS.get(t,(0,40,0,0))[1]>0)
max_roce = max(FUNDAMENTALS.get(t,(0,0,0,0))[0] for t in valid_tickers)
mid_pe   = 40   # rough sector median
mid_roce = 18

# Quadrant backgrounds
for (x0, x1, y0, y1, col, label) in [
    (0,      mid_pe,   mid_roce, max_roce+2, 'rgba(39,174,96,0.07)',  'QUALITY COMPOUNDER<br>(High ROCE, Low PE)'),
    (mid_pe, max_pe+5, mid_roce, max_roce+2, 'rgba(52,152,219,0.05)', 'GROWTH PREMIUM<br>(High ROCE, High PE)'),
    (0,      mid_pe,   0,        mid_roce,   'rgba(241,196,15,0.07)', 'VALUE TRAP?<br>(Low ROCE, Low PE)'),
    (mid_pe, max_pe+5, 0,        mid_roce,   'rgba(231,76,60,0.07)',  'AVOID<br>(Low ROCE, High PE)'),
]:
    fig.add_shape(type='rect', x0=x0, x1=x1, y0=y0, y1=y1,
                  fillcolor=col, line=dict(width=0), layer='below')
    fig.add_annotation(x=(x0+x1)/2, y=(y0+y1)/2, text=f'<i style="font-size:9px;color:#aaa">{label}</i>',
                       showarrow=False, font=dict(size=8, color='#aaa'))

# Median lines
fig.add_vline(x=mid_pe,   line_dash='dot', line_color='#adb5bd', line_width=1)
fig.add_hline(y=mid_roce, line_dash='dot', line_color='#adb5bd', line_width=1)
...
for t in valid_tickers:
    ...
    nm  = names.get(t, t)
    sec = sectors.get(t, 'Other')
    sz  = max(w/max_w * 55 + 10, 12)
    fig.add_trace(go.Scatter(
        x=[pe], y=[roce],
        mode='markers+text',
        marker=dict(color=SC.get(sec, C['navy']), size=sz, opacity=0.85,
                    line=dict(color='white', width=1.5)),
        text=[nm], textposition='top center',
        textfont=dict(size=8, color=C['navy']),
        name=sec,
        legendgroup=sec,
        showlegend=(list(sectors.values()).count(sec) ==
                    sum(1 for tt in valid_tickers if sectors.get(tt)==sec)),
        hovertemplate=(
            f'<b>{nm}</b><br>'
            f'P/E: {pe}x | ROCE: {roce}%<br>'
            f'Weight: {w*100:.1f}%<br>'
            f'Sector: {sec}'
            '<extra></extra>'
        ),
    ))
...
fig.update_layout(**BL(
    'Fundamental Quality Map - ROCE vs P/E<br>'
    '<sup>Top-left = Quality Compounder (ideal) | Bubble size = portfolio weight | Sector colour coded</sup>',
    height=540, legend_y=1.04,
))
SA(fig)
fig.update_xaxes(title_text='P/E Multiple (Valuation)', ticksuffix='x',
                 range=[0, max_pe + 8])
fig.update_yaxes(title_text='ROCE % (Quality)', ticksuffix='%',
                 range=[0, max_roce + 4])
fig.show()
```

<div class="card card-blue">
<h4>Quality Map Reading Guide</h4>
The ideal holding sits in the **top-left quadrant**: high ROCE (capital-efficient business), reasonable P/E (not overvalued). Cummins India and ABB occupy this space - the fund's quality anchors. NTPC and Power Grid appear in the mid-range (regulated ROCE ~12–16%, moderate P/E ~18×), which is appropriate for utility-grade regulated returns. The solar pure-plays (Adani Green, Waaree) show high P/E relative to current ROCE, justified by forward growth optionality - these are the highest-risk, highest-conviction names.
</div>

## 6c. Component VaR Contribution

```{python}
#| label: fig-var-contribution
#| fig-cap: "Each holding's contribution to portfolio annualised VaR 95% - Component VaR decomposition"
#| echo: false
...
fig.update_layout(**BL(
    ...
    f'w_i × ρ(r_i,r_p) × σ_i × 1.645 × √252</sup>',
    height=540, legend_y=1.04,
))
SA(fig, xgrid=True, ygrid=False)
...
if data_ok:
    ...
    fig.add_trace(go.Scatter(
        ...
        name='Rolling β', line=dict(color=C['teal'], width=2),
        hovertemplate='%{x|%d %b}<br>β: %{y:.2f}<extra></extra>',
    ), row=1, col=2)
    fig.add_hline(y=1.0, line_dash='dash', line_color=C['red'],
                  line_width=1.2, annotation_text='β=1',
...
#| fig-cap: "Rolling 90-day Pearson correlation between fund and Nifty 50 - low correlation = active management alpha regime"
#| echo: false
...
if data_ok:
    ...
    # Main line - colour-coded: green=low corr, red=high corr
    fig.add_trace(go.Scatter(
        x=corr_vals.index, y=corr_vals.values,
    ...
    fig.add_hline(y=mean_corr, line_dash='dot', line_color=C['teal'], line_width=1.8,
                  annotation_text=f'Long-run mean: {mean_corr:.2f}',
                  annotation_position='top left',
                  annotation_font=dict(size=10, color=C['teal']))
    fig.add_hline(y=0.80, line_dash='dash', line_color=C['red'], line_width=1.2,
                  annotation_text='High-beta regime (≥0.80)',
                  annotation_position='bottom right',
                  annotation_font=dict(size=9, color=C['red']))
    fig.add_hline(y=0.55, line_dash='dash', line_color=C['amber'], line_width=1.2,
                  annotation_text='Alpha regime (≤0.55)',
                  annotation_position='top right',
                  annotation_font=dict(size=9, color=C['amber']))

    fig.update_layout(**BL(
        'Rolling 90-Day Correlation: Fund vs Nifty 50<br>'
        '<sup>Periods ≤0.55 = active-management alpha regime | '
        'Periods ≥0.80 = market-beta driving returns</sup>',
        height=380,
    ))
    SA(fig)
    fig.update_xaxes(title_text='Date')
...
#| fig-cap: "Risk-return scatter - bubble size = portfolio weight; colour = sector"
#| echo: false
...
if data_ok:
    ...
    fig.update_layout(**BL(
        'Risk-Return Universe - Bubble Size = Portfolio Weight<br>'
        '<sup>Capital Market Line uses fund Sharpe ratio as slope | Jan 2020–Feb 2026</sup>',
        height=540, legend_y=1.04,
    ))
    SA(fig)
...
#| fig-cap: "Calendar heatmap: annualised sector return by year - which sectors drove alpha in which years"
#| echo: false
...
if data_ok:
    ...
    ann_tx = [[f'{v:+.1f}%' if not np.isnan(v) else '-' for v in row] for row in z_mat]

    fig = go.Figure(go.Heatmap(
        z=z_mat,
        x=[str(y) for y in cal_pivot.columns],
        y=list(cal_pivot.index),
        colorscale=[[0,'#c0392b'],[0.38,'#e67e22'],[0.5,'#f7f7f7'],[0.65,'#27ae60'],[1,'#1a6641']],
    ...
    fig.update_layout(**BL(
        'Sector Returns Calendar - Weight-Averaged Annual Return by Sector<br>'
        '<sup>Green = positive year | Red = negative year | '
        'Rows sorted by cumulative 2020–2025 return</sup>',
...
The calendar heatmap is perhaps the most actionable visualisation in the report: it shows how the energy transition theme's return drivers have rotated across sub-sectors. <strong>Solar/Wind</strong> delivered extreme alpha in 2020–21 (post-Covid RE investment surge), while <strong>Industrial/EPC</strong> re-rated in 2022–23 as order books filled. <strong>Diversified Energy</strong> (NTPC) was defensive in 2022 (the down year for the Nifty). Understanding these rotations informs dynamic rebalancing decisions.
</div>
...
The fund's factor exposures are computed from four systematic factors - **Value** (earnings yield), **Quality** (ROCE), **Momentum** (annualised return), and **Low Volatility** (inverse of annualised vol). Each factor is z-score normalised across the 23-stock universe. Portfolio-level factor tilt is the weight-averaged z-score.

## 7a. Stock-Level Factor Heatmap

```{python}
#| label: fig-factor-heatmap
#| fig-cap: "Factor score heatmap - z-scores by stock and factor; green = high score, red = low score"
#| echo: false
...
fig.update_layout(**BL(
    'Factor Exposure Heatmap - Z-Scores by Stock<br>'
...
nifty_scores = [0.0, 0.0, 0.0, 0.0]   # Nifty 50 ≈ market-neutral across factors
...
The portfolio exhibits a deliberate **Quality + Momentum** tilt. Quality (ROCE) z-score of `{python} pf_quality_str` reflects the premium given to Cummins (ROCE 38%), ABB (35%), and Power Grid (16.5% regulated return) - businesses that earn structurally above their cost of capital. The Momentum tilt (`{python} pf_momentum_str` σ) captures the energy capex cycle acceleration that began in FY2021.

The **Low Volatility** tilt (`{python} pf_lowvol_str` σ) is mildly negative - the fund accepts slightly higher volatility than the average Nifty constituent because energy transition stocks carry episodic capex-cycle volatility that is not the same as permanent capital impairment risk. The **Value** tilt (`{python} pf_value_str` σ) reflects that quality businesses in a structural growth theme trade at premium PE multiples, which is appropriate and sustainable.
</div>
...
fig.add_hline(y=0, line_color='#dee2e6', line_width=1)
fig.update_layout(**BL(
    'Stress Scenario - Portfolio NAV Impact Estimates<br>'
...
#| fig-cap: "Stock-level scenario impact matrix - all four stress scenarios simultaneously"
#| echo: false
...
fig.update_layout(**BL(
    'Scenario Impact Matrix - Top 18 Holdings by Weight<br>'
...
The **Policy Reversal** scenario produces the most asymmetric impact - highly concentrated in solar/wind pure-plays (Adani Green, Suzlon, Inox Wind). This is the fund's primary idiosyncratic risk. Mitigation: (a) anchor positions in regulated utilities (NTPC, PowerGrid) are *beneficiaries* of RE policy uncertainty as baseload demand rises, (b) industrial/EPC names (ABB, Siemens, Cummins) are policy-neutral, and (c) no single RE developer exceeds 5% of AUM.
:::

<details class="section-fold">
<summary>Scenario Methodology - Assumptions & Limitations</summary>
<div class="body">

**Scenario 1 - Market Crash (-25% Nifty):** Impact = portfolio beta × -25%. Beta is computed from 5 years of daily returns. This is a linear approximation - in actual crashes, correlations typically rise (contagion), which would make the impact worse than linear beta implies.

**Scenario 2 - Rate Shock (+150bps):** Based on interest rate sensitivity proxied by sector duration. Grid/Transmission utilities (regulated return concessions with 25–40 year asset lives) are most sensitive; industrials with shorter project cycles are less sensitive. Rule of thumb: 1% rise in discount rate reduces asset value by ~duration years × 1%.

**Scenario 3 - Commodity Spike (Steel/Copper +40%):** EPC-heavy sectors bear the burden of materials cost inflation. Industrial/EPC companies face 4–8% margin compression at the EBIT level; however, many EPC contracts include escalation clauses, limiting impact to ~50% pass-through. The 6% impact for Industrial/EPC assumes partial escalation protection.

**Scenario 4 - Policy Reversal (RE Tariff Freeze / Subsidy Cut ~30%):** Most severe for independent power producers with merchant exposure. NTPC, PowerGrid, and distribution utilities are largely insulated (regulated tariffs). RE developers (Adani Green) face discounted cash flow re-rating due to reduced PPA competitiveness.
</div>
</details>
...
#| fig-cap: "Expected return contribution per holding - based on sector P/E price targets over 3-year horizon"
#| echo: false
...
fig.update_layout(**BL(
    'Return Attribution to Price Targets - 3-Year Horizon<br>'
...
If all 23 holdings reach their sector-median P/E derived price targets over a 3-year horizon, the portfolio delivers a total return of **`{python} total_ups_str`%** - equivalent to **`{python} total_ann_str`% per annum**. This compares to the Nifty 50 GBM-implied 3-year return of approximately `{python} nifty_3yr_str`%.

The largest contributors are the high-weight anchors (NTPC, Power Grid, Cummins) combined with the meaningful upside in the mid-conviction names (ABB, Tata Power, Bhel). Key caveat: price targets are sector-median P/E based. If sector re-rating stalls - for example because of a prolonged rate cycle - the fair value multiples may compress.
</div>
...
<details class="section-fold">
<summary>Full Quantitative Scorecard - All Holdings</summary>
<div class="body">
...
fig.update_layout(
    title=dict(text='Full Quantitative Scorecard - Monarch Energy Transition Fund',
               x=0, font=dict(size=14, color=C['navy'])),
...
**Portfolio Weights:** Model-derived using a risk-quality composite scoring system (Sharpe/volatility × ROCE premium × policy anchor bonus). These weights represent target allocations in a hypothetical Rs 500 Cr AUM fund. They are not live NAV-based positions.

**Price Targets:** Derived using sector-median forward P/E multiples applied to trailing EPS (current_price × target_PE / current_PE). This is a simplified relative valuation framework. It does not incorporate DCF analysis, which would require project-level cash flow models for each regulated utility.

**Monte Carlo Simulation:** 8,000-path Geometric Brownian Motion simulation calibrated to trailing daily mean return and daily standard deviation. GBM assumes log-normal returns and constant parameters - it does not capture fat tails, regime changes, or serial correlation. The simulation is illustrative of the distribution of possible outcomes given historical parameters, not a prediction.
...
Monarch India Energy Transition Fund &nbsp;|&nbsp; Portfolio Transparency Report - February 2026 &nbsp;|&nbsp;
